...
const (
	...
	KataGoConfigFile string = "/content/model.onnx.ini"
)

func main() {
	args := os.Args[1:]
	if len(args) < 2 {
		log.Printf("ERROR usage: colab-dlshogi SSH_INFO_GOOGLE_DRIVE_FILE_ID USER_PASSWORD")
		return
	}
	fileId := args[0]
	userpassword := args[1]
